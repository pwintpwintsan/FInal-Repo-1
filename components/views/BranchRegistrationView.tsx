...
export const BranchRegistrationView: React.FC<BranchRegistrationViewProps> = ({ onBack }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-6 overflow-hidden animate-in slide-in-from-bottom duration-500">
      {/* Standardized Header */}
      <div className="w-full bg-[#292667] rounded-[2.5rem] p-6 md:p-8 text-white shadow-xl border-b-[10px] border-[#ec2027] flex flex-col md:flex-row items-center justify-between gap-6 flex-shrink-0 relative overflow-hidden">
        ...
        <div className="flex items-center gap-6 relative z-10">
           ...
           <div>
             <h2 className="text-2xl md:text-3xl font-black leading-none tracking-tight uppercase">Hub <span className="text-[#fbee21]">Registry</span></h2>
        ...
        <form onSubmit={handleSubmit} className="max-w-4xl mx-auto bg-white rounded-[3rem] p-8 md:p-12 shadow-2xl border border-slate-100 space-y-8 relative">
          {isSuccess && (
            <div className="absolute inset-0 z-50 bg-[#00a651] flex flex-col items-center justify-center text-white p-10 animate-in fade-in rounded-[3rem]">
               ...
               <p className="text-lg font-bold opacity-80 uppercase tracking-widest mt-2">Center added to global network</p>
            </div>
          )}

          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="space-y-4">
                <h3 className="text-sm font-black text-[#292667] uppercase tracking-widest mb-6 flex items-center gap-2">
                ...
                <div className="space-y-1.5">
                   ...
                   <input required type="text" className="w-full bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 focus:border-[#ec2027] outline-none font-black text-base text-[#292667] shadow-inner transition-all" placeholder="e.g. North Point Hub" />
                </div>
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Physical Location</label>
                   <input required type="text" className="w-full bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 focus:border-[#ec2027] outline-none font-black text-base text-[#292667] shadow-inner transition-all" placeholder="e.g. Mandalay City" />
                </div>
             </div>
             ...
             <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div className="space-y-1.5">
                      ...
                      <input type="number" defaultValue={10} className="w-full bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 outline-none font-black text-lg text-[#292667] text-center" />
                   </div>
                   <div className="space-y-1.5">
                      <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Learner Cap</label>
                      <input type="number" defaultValue={200} className="w-full bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 outline-none font-black text-lg text-[#292667] text-center" />
                   </div>
                </div>
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Admin Email</label>
                   <input required type="email" className="w-full bg-slate-50 px-6 py-4 rounded-2xl border border-slate-100 focus:border-[#ec2027] outline-none font-black text-base text-[#292667] shadow-inner transition-all" placeholder="admin@hub.ubook.com" />
                </div>
             </div>
          </div>

          <button type="submit" className="w-full py-6 bg-[#292667] text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-xl hover:bg-[#00a651] transition-all border-b-6 border-black/10 active:scale-95">
