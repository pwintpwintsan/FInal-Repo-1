...
export const CourseViewerView: React.FC<CourseViewerViewProps> = ({ courseId, onBack }) => {
  ...
  const optionColors = [
    ...
    { bg: 'bg-amber-50', text: 'text-amber-600', active: 'bg-amber-600', ring: 'ring-amber-100', border: 'border-amber-100' },
        ...
        <div className="flex items-center gap-3 relative z-10">
           ...
           <div>
             ...
             <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-[8px] font-black text-[#fbee21] uppercase tracking-widest opacity-80">{course.category}</span>
             </div>
           </div>
        </div>
        <div className="flex items-center gap-6 px-4 md:border-l-2 border-white/10 relative z-10">
           <div className="text-center">
              <p className="text-xl font-black text-[#fbee21]">65%</p>
              ...
              {course.modules.map((mod, modIdx) => (
                <div key={mod.id} className="relative">
                  <div className="flex items-center gap-2 mb-2">
                    <div className="w-7 h-7 rounded-lg bg-[#292667] text-[#fbee21] flex items-center justify-center font-black text-[9px] shadow-sm border-b-2 border-black/10 flex-shrink-0">{modIdx + 1}</div>
                    ...
                    {mod.lessons.map((lesson) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          <p className={`text-[6px] font-bold uppercase tracking-widest mt-0.5 ${activeLesson?.id === lesson.id ? 'text-[#fbee21]' : 'text-slate-300'}`}>{lesson.type}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                    ...
                    <div className="space-y-6 animate-in slide-in-from-bottom-4">
                       <div className="aspect-video w-full bg-slate-900 rounded-2xl shadow-xl flex items-center justify-center relative group cursor-pointer overflow-hidden border-4 border-slate-50 ring-1 ring-slate-100">
                          ...
                          <div className="absolute bottom-4 left-4 z-10">
                             <span className="px-3 py-1 bg-[#fbee21] text-[#292667] rounded-lg font-black text-[8px] uppercase tracking-widest shadow-md">Premium 4K Stream</span>
                          </div>
                       </div>
                       ...
                       {!isSubmitted ? (
                         <>
                           <div className="text-center space-y-2">
                              <div className="w-16 h-16 bg-amber-50 text-amber-500 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-amber-100 border-b-2 border-black/10 rotate-3">
                                 <Zap size={32} fill="currentColor" />
                              </div>
                              <h4 className="text-2xl font-black text-[#292667] uppercase tracking-tighter">Quest Log Selection</h4>
                              <p className="text-[9px] font-black text-amber-600 uppercase tracking-widest">Select your answer and submit it for review</p>
                           </div>
                         ...
                         <div className="text-center py-12 animate-in zoom-in-95 duration-500">
                            ...
                            <button 
                              ...
                              className="px-8 py-3 bg-[#292667] text-[#fbee21] rounded-xl font-black text-[9px] uppercase tracking-[0.2em] shadow-lg hover:bg-[#00a651] hover:text-white transition-all border-b-4 border-black/10 active:scale-95"
                            >
                               Next Quest
                            </button>
                         </div>
                       ...
                       {!isSubmitted ? (
                         <>
                           ...
                           <div className="p-8 bg-slate-50 rounded-[2.5rem] border-2 border-white shadow-inner flex flex-col items-center gap-4 text-center relative overflow-hidden">
                              ...
                              <button onClick={handleFileUpload} className="px-8 py-4 bg-[#292667] text-[#fbee21] rounded-2xl font-black text-[10px] uppercase tracking-[0.2em] shadow-lg hover:bg-[#00a651] transition-all border-b-4 border-black/10 active:scale-95 flex items-center gap-3 group">
