    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-[#292667]/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border-t-8 border-[#00a651] relative animate-in zoom-in-95 duration-300 max-h-[85vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 md:p-8 bg-slate-50 border-b border-slate-100 shrink-0">
          ...
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="relative shrink-0">
              ...
              <div className="absolute -bottom-2 -right-2 bg-[#fbee21] text-[#292667] p-1.5 rounded-lg shadow-md rotate-12 border-2 border-white">
        ...
        <div className="p-6 border-t border-slate-100 bg-white shrink-0">
          <button onClick={onClose} className="w-full py-4 bg-[#292667] text-[#fbee21] rounded-xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-[#ec2027] transition-all border-b-4 border-black/10 active:scale-95">
             Close Dashboard
          </button>
        </div>
      </div>
...
export const GradesView: React.FC = () => {
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="w-full bg-[#292667] rounded-2xl p-6 text-white shadow-xl border-b-8 border-[#fbee21] flex flex-col md:flex-row items-center justify-between gap-4 flex-shrink-0 relative overflow-hidden">
        <div className="flex items-center gap-4 relative z-10">
           <div className="p-3.5 bg-[#fbee21] rounded-xl text-[#292667] shadow-lg rotate-3 border-b-4 border-black/10">
             <GraduationCap size={28} strokeWidth={3} />
           </div>
           <div>
             <h2 className="text-2xl font-black leading-none tracking-tight uppercase">Exam <span className="text-[#fbee21]">Results</span></h2>
             <p className="text-[10px] font-black uppercase tracking-widest text-white/40 mt-1">Assessment Engine</p>
           </div>
        </div>
        <button onClick={() => alert('Exporting...')} className="px-6 py-2.5 bg-white/10 hover:bg-[#ec2027] text-white rounded-xl transition-all border border-white/20 active:scale-95 text-[10px] font-black uppercase tracking-widest z-10">
           <Download size={18} className="inline-block mr-2" /> Export PDF
        </button>
      </div>
      ...
      <div className="flex-1 bg-white rounded-3xl border border-slate-100 overflow-hidden shadow-xl flex flex-col mb-2">
        <div className="px-6 py-4 bg-slate-50 border-b border-slate-100 flex items-center justify-between shrink-0">
           ...
           <div className="flex items-center gap-3">
              <div className="flex items-center gap-1.5 bg-[#292667] px-3 py-1.5 rounded-lg text-white">
                 <Percent size={12} className="text-[#fbee21]" />
        ...
        <div className="flex-1 overflow-y-auto scrollbar-hide divide-y divide-slate-50">
          {filteredStudents.map((student) => {
            ...
            return (
              <div key={student.id} className="px-6 py-4 flex items-center justify-between hover:bg-slate-50 transition-all border-l-4 border-transparent hover:border-[#fbee21]">
