...
export const AccountCreationView: React.FC<AccountCreationViewProps> = ({ checkPermission }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden animate-in fade-in duration-500">
      {/* Standardized Header - Provision Switch Removed */}
      <div className="w-full bg-[#292667] rounded-2xl p-5 md:p-6 text-white shadow-xl border-b-8 border-[#f43f5e] flex flex-col md:flex-row items-center justify-between gap-4 flex-shrink-0 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 bg-white/5 rounded-full -mr-24 -mt-24 blur-3xl"></div>
        <div className="flex items-center gap-4 relative z-10">
           <div className="p-3.5 bg-[#f43f5e] rounded-xl text-white shadow-lg rotate-3">
             <UserPlus size={28} strokeWidth={3} />
           </div>
           <div>
             <h2 className="text-xl md:text-2xl font-black leading-none tracking-tight uppercase">Staff <span className="text-[#fbee21]">Directory</span></h2>
        ...
        <div className="flex items-center gap-4 relative z-10">
          <div className="bg-white/5 px-4 py-2 rounded-xl border border-white/10 backdrop-blur-md hidden sm:block">
            <span className="text-[10px] font-black uppercase tracking-widest text-[#fbee21]">{mockStaff.length} ACTIVE USERS</span>
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-hidden flex flex-col">
        <div className="flex-1 bg-white rounded-3xl border border-slate-100 shadow-xl flex flex-col overflow-hidden">
          ...
          <div className="p-3 border-b border-slate-100 bg-slate-50/30 flex items-center">
             <div className="flex-1 flex items-center gap-3 bg-white px-4 py-2.5 rounded-xl border border-slate-100 w-full focus-within:border-[#f43f5e] transition-all shadow-sm">
               <Search size={18} className="text-slate-300" strokeWidth={3} />
               <input placeholder="Search members by name or email..." className="bg-transparent outline-none w-full font-black text-[#292667] text-xs placeholder:text-slate-200 uppercase" />
             </div>
          </div>
          
          <div className="flex-1 overflow-y-auto scrollbar-hide">
            <table className="w-full text-left">
              <thead className="sticky top-0 bg-[#292667] text-white text-[9px] font-black uppercase tracking-widest z-20">
                <tr>
              ...
              <tbody className="divide-y divide-slate-50">
                {mockStaff.map((staff) => (
                  <tr key={staff.id} className="hover:bg-slate-50/50 transition-colors group">
                    ...
                    <td className="px-8 py-4 text-[10px] font-black text-[#292667] uppercase tracking-tight truncate max-w-[180px]">{staff.branch}</td>
                    <td className="px-8 py-4">
                    ...
                    <td className="px-8 py-4 text-right">
                       <div className="flex justify-end gap-1">
                         {canEdit && (
                           <button className="p-2 text-slate-300 hover:text-[#292667] transition-all"><Edit size={16} strokeWidth={3} /></button>
                         )}
                         {canDelete && (
                           <button className="p-2 text-slate-300 hover:text-[#f43f5e] transition-all"><Trash2 size={16} strokeWidth={3} /></button>
                         )}
                       </div>
                    </td>
                  </tr>
