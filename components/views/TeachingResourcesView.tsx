...
const ResourceViewer = ({ resource, onClose }: { resource: any, onClose: () => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10 bg-[#292667]/90 backdrop-blur-xl animate-in fade-in duration-300">
      <div className="bg-white rounded-[2rem] w-full max-w-4xl h-full max-h-[80vh] shadow-[0_40px_100px_rgba(0,0,0,0.5)] border-t-[8px] border-[#6366f1] flex flex-col overflow-hidden animate-in zoom-in-95 duration-300">
        
        <div className="p-4 border-b border-slate-100 flex items-center justify-between shrink-0 bg-slate-50/50">
          <div className="flex items-center gap-3">
             <div className={`p-2.5 rounded-xl text-white shadow-lg ${isVideo ? 'bg-indigo-500' : 'bg-rose-500'}`}>
               {isVideo ? <MonitorPlay size={20} strokeWidth={3} /> : <FileText size={20} strokeWidth={3} />}
             </div>
             <div>
               <h2 className="text-base font-black text-[#292667] uppercase tracking-tighter leading-none">{resource.title}</h2>
          ...
          {isVideo ? (
            <div className="w-full h-full flex items-center justify-center relative">
               ...
               <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20 w-1/2 h-1 bg-white/10 rounded-full overflow-hidden">
                  <div className="h-full bg-[#fbee21] w-1/3 rounded-full"></div>
               </div>
            </div>
          ) : (
        ...
        <div className="p-4 bg-white border-t border-slate-100 flex items-center justify-between shrink-0">
           ...
           <button onClick={onClose} className="px-6 py-3 bg-[#292667] text-[#fbee21] rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-[#6366f1] hover:text-white transition-all shadow-xl active:scale-95">Return</button>
        </div>
      </div>
    </div>
  );
...
const UploadAssetModal = ({ onClose, onUpload }: { onClose: () => void, onUpload: (asset: any) => void }) => {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-[#292667]/80 backdrop-blur-md animate-in fade-in duration-300">
      <div className="bg-white rounded-[2rem] p-5 md:p-6 max-w-lg w-full shadow-2xl border-t-[8px] border-[#6366f1] relative animate-in zoom-in-95 duration-300 max-h-[90vh] overflow-y-auto scrollbar-hide">
        ...
        <div className="flex items-center gap-3 mb-5">
          <div className="p-2.5 bg-[#6366f1] text-white rounded-xl shadow-xl">
            <FileUp size={20} strokeWidth={3} />
          </div>
          <div>
            <h3 className="text-lg font-black text-[#292667] uppercase tracking-tighter leading-none">Upload Asset</h3>
        ...
        <div className="space-y-3.5">
          <div className="space-y-1">
            ...
            <input 
              ...
              className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black text-[#292667] text-xs outline-none focus:border-[#6366f1] transition-all"
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            <div className="space-y-1">
              ...
              <select 
                className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black text-[10px] text-[#292667] outline-none appearance-none uppercase"
                value={formData.type}
            ...
            <div className="space-y-1">
              <label className="text-[8px] font-black text-slate-400 uppercase tracking-widest ml-1">Series</label>
              <select 
                className="w-full px-4 py-2.5 bg-slate-50 border-2 border-slate-100 rounded-xl font-black text-[10px] text-[#292667] outline-none appearance-none uppercase"
                value={formData.book}
        ...
        <div className="flex flex-col sm:flex-row gap-2.5 mt-6">
          ...
          <button 
            ...
            className="flex-[2] py-3 px-4 bg-[#292667] text-[#fbee21] rounded-xl font-black text-[9px] uppercase tracking-widest flex items-center justify-center gap-1.5 hover:bg-[#6366f1] hover:text-white shadow-lg transition-all active:scale-95"
          >
...
export const TeachingResourcesView: React.FC<TeachingResourcesViewProps> = ({ checkPermission }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-3 overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="w-full bg-[#292667] rounded-xl p-4 md:p-5 text-white shadow-xl border-b-6 border-[#6366f1] flex flex-col md:flex-row items-center justify-between gap-4 flex-shrink-0 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-white/5 rounded-full -mr-16 -mt-16 blur-3xl"></div>
        <div className="flex items-center gap-3 relative z-10">
           <div className="p-2.5 bg-[#6366f1] rounded-lg text-white shadow-xl rotate-3">
             <FileSearch size={22} strokeWidth={3} />
           </div>
           <div>
             <h2 className="text-lg md:text-xl font-black leading-none tracking-tight uppercase">Teaching <span className="text-[#fbee21]">Library</span></h2>
        ...
        {canUpload && (
          <button 
            ...
            className="flex items-center gap-2 px-5 py-2.5 bg-[#fbee21] text-[#292667] rounded-lg font-black text-[9px] uppercase tracking-widest shadow-xl hover:scale-105 active:scale-95 transition-all border-b-4 border-black/10 relative z-10"
          >
      ...
      <div className="w-full bg-white p-2 rounded-xl shadow-lg border border-slate-100 flex flex-col md:flex-row items-center gap-2 flex-shrink-0">
        <div className="flex-[2] flex items-center gap-2.5 bg-slate-50 px-4 py-2 rounded-lg border border-slate-100 w-full group focus-within:border-[#6366f1] transition-all">
          <Search size={16} className="text-slate-400 group-focus-within:text-[#6366f1]" strokeWidth={3} />
          <input 
            type="text" 
            placeholder="Search hub library..." 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="bg-transparent text-[11px] font-black text-[#292667] outline-none w-full placeholder:text-slate-300 uppercase"
          />
        </div>
        <div className="flex-1 relative w-full md:w-auto">
          <select 
            ...
            className="w-full bg-slate-50 pl-3 pr-8 py-2 rounded-lg border border-slate-100 outline-none font-black text-[8px] text-[#292667] uppercase appearance-none cursor-pointer"
          >
      ...
      <div className="flex-1 overflow-y-auto scrollbar-hide pb-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {filteredResources.map((res) => (
            <div key={res.id} className="bg-white rounded-2xl p-4 md:p-5 border-2 border-slate-50 hover:border-[#6366f1]/20 transition-all group shadow-md flex items-start gap-3 relative overflow-hidden">
              <div className={`w-10 h-10 md:w-11 md:h-11 rounded-lg flex items-center justify-center flex-shrink-0 transition-transform group-hover:rotate-6 shadow-md ${res.type === 'Video' ? 'bg-indigo-100 text-indigo-600' : 'bg-red-100 text-[#ec2027]'}`}>
                {res.type === 'Video' ? <PlayCircle size={18} strokeWidth={2.5} /> : <FileText size={18} strokeWidth={2.5} />}
              </div>
              
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5 mb-1">
                  <span className="text-[6px] font-black uppercase tracking-widest text-[#ec2027] bg-red-50 px-1.5 py-0.5 rounded-full">{res.type}</span>
                  <span className="text-[6px] font-black text-slate-300 uppercase tracking-widest">{res.size}</span>
                </div>
                <h4 className="text-[11px] font-black text-[#292667] uppercase tracking-tight leading-snug group-hover:text-[#6366f1] transition-colors line-clamp-1">{res.title}</h4>
              ...
              <div className="flex flex-col gap-1">
                <button 
                   ...
                   className="p-2.5 bg-indigo-50 text-indigo-600 rounded-lg shadow-sm hover:bg-[#292667] hover:text-white transition-all active:scale-90 flex items-center gap-1.5 group/btn"
                   title="View Asset"
                >
                   <Eye size={12} strokeWidth={3} className="group-hover/btn:scale-110 transition-transform" />
                   <span className="text-[7px] font-black uppercase tracking-widest hidden xl:block">Preview</span>
                </button>
                {canDelete && (
                  <button 
                    onClick={() => handleDelete(res.id)}
                    className="p-2 bg-slate-50 text-slate-400 hover:text-white hover:bg-[#ec2027] rounded-lg transition-all active:scale-90"
                  >
          ...
          {filteredResources.length === 0 && (
             <div className="col-span-full py-12 text-center opacity-20">
                <FileSearch size={48} className="mx-auto mb-3" strokeWidth={1} />
                <h4 className="text-lg font-black uppercase tracking-widest">Library Search Empty</h4>
                <p className="text-[9px] font-bold uppercase tracking-[0.2em] mt-1">Adjust filters</p>
             </div>
          )}
        </div>
      </div>
