...
const UnlockModal = ({ courseName, onClose, onPurchase }: { courseName: string, onClose: () => void, onPurchase?: () => void }) => (
  <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-[#292667]/90 backdrop-blur-xl animate-in fade-in duration-300">
    <div className="bg-white rounded-3xl p-8 md:p-10 max-w-md w-full shadow-2xl border-t-[10px] border-[#ec2027] text-center animate-in zoom-in-95 duration-300 relative">
      ...
      <h3 className="text-xl font-black text-[#292667] mb-1 uppercase tracking-tight">Unlock Course</h3>
      ...
      <div className="space-y-3">
        <button 
          ...
          className="w-full py-4 bg-[#292667] text-[#fbee21] rounded-xl font-black text-xs uppercase tracking-widest hover:bg-[#00a651] hover:text-white transition-all shadow-xl flex items-center justify-center gap-3 border-b-4 border-black/10 active:scale-95"
        >
...
export const MyClassesView: React.FC<MyClassesViewProps> = ({ teacher, classes, activeRole, onEnterClass, onEnterCenter, onEnterCourse, onEditCourse, onAddBranch, onPurchaseRedirect }) => {
  ...
  return (
    <div className="h-full flex flex-col gap-4 overflow-hidden">
      ...
      <div className="w-full bg-[#292667] rounded-2xl p-5 md:p-6 text-white shadow-xl border-b-8 border-[#ec2027] flex flex-col md:flex-row items-center justify-between gap-4 flex-shrink-0 relative overflow-hidden">
        <div className="absolute top-0 right-0 w-80 h-80 bg-white/5 rounded-full -mr-24 -mt-24 blur-3xl"></div>
        <div className="flex items-center gap-4 relative z-10">
           <div className={`p-3 md:p-4 rounded-xl shadow-lg ${isAdmin ? 'bg-[#ec2027]' : 'bg-[#fbee21] text-[#292667]'}`}>
           ...
           <div>
             <h2 className="text-xl md:text-2xl font-black leading-none tracking-tight uppercase">
               Course <span className="text-[#fbee21]">{isAdmin ? 'Library' : 'Portal'}</span>
             </h2>
        ...
        <div className="flex items-center gap-6 relative z-10 bg-white/5 px-5 py-3 rounded-2xl border border-white/10 backdrop-blur-sm">
             <div className="text-center">
                <p className="text-2xl font-black text-[#fbee21] leading-none mb-0.5">{filteredCourses.length}</p>
             ...
             {isAdmin && (
               <button onClick={() => {}} className="p-2.5 bg-[#fbee21] text-[#292667] rounded-lg font-black shadow-lg hover:scale-105 active:scale-95 transition-all ml-2" title="Create Course">
      ...
      <div className="w-full bg-white p-3 rounded-2xl shadow-lg border border-slate-100 flex flex-col md:flex-row items-center gap-3 flex-shrink-0">
        <div className="flex-1 flex items-center gap-3 bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 w-full group focus-within:border-[#ec2027] transition-all">
          ...
          <input 
            ...
            className="bg-transparent text-sm font-black text-[#292667] outline-none w-full placeholder:text-slate-200 uppercase"
          />
        </div>
      </div>
            ...
            >
              ...
              <div className="p-6 md:p-8 flex flex-col flex-1 min-w-0">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                  <div>
                    <h3 className="text-2xl font-black text-[#292667] uppercase leading-tight group-hover:text-[#ec2027] transition-colors mb-1">{course.name}</h3>
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                     <div className="bg-slate-50 px-4 py-2 rounded-xl flex items-center gap-2 border border-slate-100">
                        ...
                        <span className="text-xs font-black text-[#292667]">{course.duration || '20h'}</span>
                     </div>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-[#292667]">{course.modules.reduce((a, b) => a + b.lessons.length, 0)} Units</p>
                    </div>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-[#292667]">1.2k+</p>
                    </div>
                  </div>
                  <div className="bg-slate-50 p-3 rounded-2xl border border-slate-100 flex items-center gap-3 hidden sm:flex">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-black text-[#292667]">{course.level || 'Foundation'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="mt-auto pt-6 border-t border-slate-50 flex items-center justify-between">
                   ...
                   <div className="flex items-center gap-2 px-4 py-2 bg-[#292667] text-white rounded-xl shadow-lg opacity-0 group-hover:opacity-100 transition-all">
